...
import org.firstinspires.ftc.teamcode.RobotFunctions.GoBildaPinpointDriver;
...
public class Test2 extends Movable {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.x) {            // X = 410.246, Y = -657.651, H = 0
                driveTo(300, 0, 0);
            }
    ...
    public void driveTo(double targetX, double targetY, double targetH) { // h in degrees, not radians because only psychos like radians

        // ==== TUNABLE CONSTANTS ====
        double POSITION_K = 0.003;       // proportional gain for X/Y
        double HEADING_K  = 0.01;        // proportional gain for heading

        double POSITION_TOLERANCE_MM = 10;   // stopping tolerance for X/Y
        double HEADING_TOLERANCE_DEG = 2;    // stopping tolerance for heading
        // ============================
        ...
        while (opModeIsActive()) {
            ...
            if (Math.abs(errorX) < POSITION_TOLERANCE_MM &&
                    ...
                    Math.abs(errorH) < HEADING_TOLERANCE_DEG) {
            ...
            double headingRad = Math.toRadians(currentH);
            ...
            double driveX = robotX * POSITION_K;
            double driveY = robotY * POSITION_K;
            double turn   = errorH * HEADING_K;

            // --- Limit power ---
            driveX = Range.clip(driveX, -0.7, 0.7);
            driveY = Range.clip(driveY, -0.7, 0.7);
    ...
    public void drive(double x, double y, double turn) {
        FLW.setPower(x + y + turn);
        FRW.setPower(x - y - turn);
        BLW.setPower(x - y + turn);
        BRW.setPower(x + y - turn);
    }
}